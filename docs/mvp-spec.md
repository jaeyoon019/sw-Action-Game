...
- PR 머지 전 체크리스트:
  ...
  - [ ] MVP 제외 항목이 코드에 없음
  - [ ] 자기 담당 파일만 수정
- 커밋 메시지: 한국어 가능. 단, "작업중", "수정", "버그" 같은 모호한 메시지 금지
- AI 사용 시 이 문서 전체를 첨부하고 **"이 스펙 안에서만 구현해줘"** 명시
- AI 사용 기록은 `docs/ai-usage-record/<이름>.md`에 누적 (날짜 / 프롬프트 요약 / 결과 요약 / 채택 여부)
...
- [ ] 팀원 5명 PC에서 평균 60FPS 유지 (FPS 카운터로 확인)
- [ ] Windows에서 실행 검증 (macOS/Linux는 권장이지만 MVP 필수 아님)
- [ ] 2.2의 제외 항목이 코드에 **하나도 없음**
- [ ] `docs/ai-usage-record/<이름>.md`가 팀원 전원에 대해 채워져 있음

---

## 11. AI 사용 가이드 (실전)

팀원이 AI에 코드를 요청할 때 사용할 표준 프롬프트 템플릿:

```
첨부 문서: docs/mvp-spec.md (전체)

내가 작업할 파일은 [파일 경로]야.
이 스펙 문서 안에서만 구현해줘. 다음 사항을 반드시 지킬 것:

1. 폴더 구조와 파일 경로는 4번 항목 그대로
2. 상수는 settings.py에서 import해서 쓰고 새로 만들지 말 것
3. 7번의 클래스 시그니처와 정확히 일치
4. 2.2의 제외 항목(인증, 상점, 인벤토리, 점수, 세이브 등)은 단 한 줄도 포함하지 말 것
5. 지정된 파일 외에는 수정 금지 (import만 가능)
6. 모든 이동/물리는 dt 기반

요청: [구체적으로 무엇을 만들어 달라는지]
```
